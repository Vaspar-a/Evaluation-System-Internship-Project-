...
class TableLayout extends Component {
  ...
  reRender(theadData, tbodyData, pedWeightage) {
    this["table"].current.reRender(theadData, tbodyData, pedWeightage);
    this.setState({
    ...
    // this["trashBar"].current.reRender(data);
  }

  getFilterElements() {
    let result = this.state.heading.map((ele, index) => {
      //console.log(ele);
    return ( 
      <div className={'field'}>
        <p className={'filterPara'} id={`p${index}`} ref={this[`p${index}`]} >{ele}</p> 
        <input className={'filterInput'} type="text" id={`i${index}`} ref={this[`i${index}`]} />
      </div> 
      );
    });
  ...
  render() {
    return (
      <div className={`tableLayoutResult`}>
        <div className={"coverUp"} ref={this["coverUp"]} onClick={this.setModalIsOpen}>
          <svg
            aria-hidden="true"
            focusable="false"
            data-prefix="fas"
        ...
        <Table ref={this["table"]} data={this.props.data} filterTrashCan={this.filterTrashCanInTableLayout} />
      </div>
    );
  }
}
